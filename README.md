...
`WebApplication`, `Person`, `BreadcrumbList` on stop and line pages, `BusStop` on stop pages,
`BusTrip` with its itinerary on line pages), `robots.txt` and multi-locale sitemaps
...
`lastmod` on stop and line URLs is the date that entity's own content last changed — its name,
or the lines calling there — tracked by fingerprint in `server/utils/referenceData.ts`. It is
deliberately *not* the date of the request: a sitemap where every URL changed today is one
search engines learn to ignore, which is also why `autoLastmod` is left off.

Nothing is prerendered. These pages are driven by the live stop and line catalogue, and
prerendering baked whatever SIRI was serving at build time into static files — including
sitemaps that stayed frozen until the next deploy.
